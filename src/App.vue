...
<div class="relative">
<button
@click="togglePosProfileDropdown"
...
<div v-if="showPosProfileDropdown" class="absolute z-10 w-full mt-1 bg-white rounded-md shadow-lg border border-gray-200 max-h-48 overflow-y-auto">
<div
v-for="profile in posProfiles"
:key="profile.id"
...
<button
@click="confirmPosProfile"
:disabled="!canConfirmPosProfile"
:class="[
...
<!-- SEO Meta Tags -->
<div v-if="false">
<meta name="description" content="FoodiePoint POS - Advanced Restaurant Management System. Order management, table booking, and payment processing made easy.">
...
<div class="flex flex-col md:flex-row items-center space-y-2 md:space-y-0 md:space-x-6 w-full md:w-auto" :class="{ 'hidden': !showMobileMenu, 'md:flex': true }">
...
<div
v-for="table in selectedRoom.tables"
:key="table.id"
...
<!-- Grid View -->
<div v-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 overflow-y-auto max-h-[500px]">
<div
...
<button class="text-indigo-600 hover:text-indigo-800 cursor-pointer !rounded-button whitespace-nowrap">
...
<button class="ml-3 text-indigo-600 hover:text-indigo-800 cursor-pointer !rounded-button whitespace-nowrap">
...
<div v-if="showCustomerDropdown" class="absolute z-10 w-full mt-1 bg-white rounded-md shadow-lg border border-gray-200 max-h-48 overflow-y-auto">
<div
...
<div class="relative flex-1">
<button
@click="showWaiterDropdown = !showWaiterDropdown"
class="w-full px-3 py-2 bg-gray-50 rounded-md text-left text-sm flex items-center justify-between border border-gray-200 !rounded-button">
<span>{{ selectedWaiter ? selectedWaiter.name : 'Select Waiter' }}</span>
<i class="fas fa-chevron-down text-gray-400"></i>
</button>
<div v-if="showWaiterDropdown" class="absolute z-10 w-full mt-1 bg-white rounded-md shadow-lg border border-gray-200 max-h-48 overflow-y-auto">
<div
...
<div v-if="currentOrder.items.length === 0" class="flex flex-col items-center justify-center h-40 text-gray-400">
...
>
<i :class="['text-xl mb-1', method.icon, selectedPaymentMethod === method.id ? 'text-indigo-600' : 'text-gray-600']"></i>
<span :class="selectedPaymentMethod === method.id ? 'text-indigo-600' : 'text-gray-600'">{{ method.name }}</span>
</button>
</div>
</div>
<div class="mb-4">
...
>
<button class="bg-gray-200 px-4 rounded-r-md text-gray-700 !rounded-button whitespace-nowrap">Apply</button>
</div>
</div>
</div>
<div class="flex space-x-3">
